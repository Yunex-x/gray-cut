  ...
  return (
    <section className={`w-full bg-[#F6F4F1] ${className}`}>
      <div className="mx-auto px-4 md:px-8 lg:px-10 py-12 md:py-16 lg:py-20">
        {/* Header */}
        <div className="w-full mx-auto flex flex-col justify-end items-start gap-1 md:gap-2 mb-8 md:mb-12 lg:mb-14">
          <h2 className="w-full font-playfair font-semibold text-[32px] md:text-[48px] lg:text-[64px] leading-[42px] md:leading-[64px] lg:leading-[85px] text-center tracking-[-0.02em] uppercase text-[#2C2D30]">
            {title}
          </h2>

          <p className="w-full font-poppins font-normal text-sm md:text-base leading-5 md:leading-6 text-center uppercase text-[rgba(44,45,48,0.7)] px-4">
            {subtitle}
          </p>
        </div>
        ...
        >
          {steps.map((step, index) => (
            ...
            >
              ...
              <div className="relative w-full h-[166px] bg-[#F9F9F9] overflow-hidden">
                <Image
                  src={step.image}
                  alt={`${step.label} step`}
                  fill
